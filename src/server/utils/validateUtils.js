...
const validateNewInterval = (newIntervalSeconds) => {
  ...
  const rateIndex = config.intervalRates.findIndex(
    (i) => i === numberNewIntervalSeconds
  );
  if (rateIndex < 0) {
  ...
  if (
    moment.unix(config.intervalStartCreateTimestamp) < numberNewIntervalSeconds
  ) {
...
const validateTimestamp = (fromTimestamp) => {
  ...
  if (
    ...
    moment.unix(config.intervalStartCreateTimestamp)
  ) {
