  ...
  const createData = () => {
    ...
    for (let i = 0; i < config.defaultEventRate; i++) {
      ...
      const number = Math.floor(
        Math.random() * Math.floor(config.maxRandomNumber)
      );
    ...
    const intervalStartCreateTimestamp = app.get(
      'intervalStartCreateTimestamp'
    );
...
const reCalculateCandlesticks = (app, lastProcessedTimestamp = 0) => {
  ...
  if (newCandlesticks.length > config.maxCandlesticksCountEmit) {
    newCandlesticks.splice(
      0,
      newCandlesticks.length - config.maxCandlesticksCountEmit
    );
  }
