...
export const updateInterval = async (selectedInterval) => {
  ...
  try {
    newDisplayData = await api.post(
      `api/data/updateInterval?interval=${selectedInterval}`
    );
  } catch (err) {}
...
export const getHistoryData = async (historyTimestamp) => {
  let newDisplayData = null;
  try {
    newDisplayData = await api.get(
      `api/data/getHistoryData?timestamp=${historyTimestamp}`
    );
  } catch (err) {}
  return newDisplayData;
};
